...
def main(args):
    if not os.path.exists(args.path_to_MSRVTT_captions):
        os.makedirs(args.path_to_MSRVTT_captions)
    ...
    msrvtt_dataset = MSRVTT_Captioning_Dataset(os.path.join(args.path_to_MSRVTT, "TrainValVideo"))
    ...
    for video_id, cap in tqdm(msrvtt_dataset):
        if args.resume:
            if os.path.exists(os.path.join(args.path_to_MSRVTT_captions, f"{video_id}.txt")):
                continue
        ...
        try:
            ...
            output_filename = os.path.join(args.path_to_MSRVTT_captions, f"{video_id}.txt")
